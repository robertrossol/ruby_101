...
def add_word_size(word)
  word + " " + word.size.to_s
end

def word_lengths(str)
  str.split.map { |word| word + " #{word.length}" } #original attempt
  # str.split.map { |word| "#{word} #{word.length}" }
  # str.split.map { |word| word + " " + word.length.to_s }
  # str.split.map { |word| add_word_size(word) } #alternate approach with helper method above
end
